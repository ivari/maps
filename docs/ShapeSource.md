...
| clusterProperties | `object` | `none` | `false` | Specifies custom properties on the generated clusters if clustering is enabled, aggregating values from clustered points.<br/>Has the form `{ 'property_name': [operator, map_expression] }`. |
